...
function validateEnvironment(): void {
  const manualCaptcha = yn(process.env.CAPTCHA_MANUAL, { default: false });
  const requiredVars: Record<string, string | undefined> = {
    SUNO_COOKIE: process.env.SUNO_COOKIE,
    ...(!manualCaptcha && { TWOCAPTCHA_KEY: process.env.TWOCAPTCHA_KEY })
  };
...
class SunoApi {
  ...
  private async launchBrowser(): Promise<BrowserContext> {
    ...
    const manualMode = yn(process.env.CAPTCHA_MANUAL, { default: false });
    const browser = await this.getBrowserType().launch({
      args,
      headless: manualMode ? false : yn(process.env.BROWSER_HEADLESS, { default: true })
    });
  ...
  public async getCaptcha(): Promise<string|null> {
    ...
    const captchaSolvingPromise = new Promise<void>(async (resolve, reject) => {
      ...
      try {
        ...
        // Manual CAPTCHA mode: wait for user to solve in browser window
        if (yn(process.env.CAPTCHA_MANUAL, { default: false })) {
          logger.info('Manual CAPTCHA mode: please solve the CAPTCHA in the browser window');
          return;
        }
